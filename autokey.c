...
char cipher_text[14],plain_text[14],key[14],concat_arr[14];
int num_key[14],num_pt[14],num_ct[14];
int p_length=0,k_length=0,concat_length=0;
...
printf("\nEnter the key\t");
	...
	k_length=strlen(key);
	concat_length=p_length-k_length;
	

	for(i=0;i<concat_length;++i)
	{
        concat_arr[i]=plain_text[i];
	
	}
    
    strcat(key,concat_arr);

    printf("\nThe modified key is=%s\n",key);
